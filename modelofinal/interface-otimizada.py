...
# --- MODO DE OTIMIZAÇÃO ---
# Mude para True para desligar a visualização e ganhar desempenho máximo
# Mude para False para depurar e ver os retângulos na tela
MODO_COMPETICAO = True 
...
FRAME_WIDTH, FRAME_HEIGHT = 320, 240 # Manter a resolução baixa

# ... (O resto do seu código, classes App, TelaInicio, TelaCalibracao, permanece o mesmo) ...
# (Cole aqui as classes inalteradas para economizar espaço)
class App:
    def __init__(self):
        pygame.init(); self.screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT)); pygame.display.set_caption("Axiom Robô Vision")
        self.clock = pygame.time.Clock(); self.running = True; self.state = 'inicio'
        try:
            logo_original = pygame.image.load('logo.png').convert_alpha(); self.logo = pygame.transform.scale(logo_original, (150, 150)); self.logo_pequeno = pygame.transform.scale(logo_original, (100, 100))
        except pygame.error: self.logo = pygame.Surface((150, 150), pygame.SRCALPHA); self.logo_pequeno = pygame.Surface((100, 100), pygame.SRCALPHA)
        try:
            self.font_grande = pygame.font.Font('Montserrat-Bold.ttf', 30); self.font_media_bold = pygame.font.Font('Montserrat-Bold.ttf', 26); self.font_media = pygame.font.Font('Montserrat-Bold.ttf', 24)
        except FileNotFoundError: self.font_grande = pygame.font.Font(None, 46); self.font_media_bold = pygame.font.Font(None, 40); self.font_media = pygame.font.Font(None, 36)
        self.tela_inicio = TelaInicio(self); self.tela_calibracao = TelaCalibracao(self); self.tela_rodada = TelaRodada(self)
        self.calib_vars = {'THRESHOLD_VALUE': 80, 'WHITE_THRESHOLD_LOWER': 200, 'LOWER_GREEN': np.array([40, 50, 50]), 'UPPER_GREEN': np.array([80, 255, 255]), 'LOWER_RED1': np.array([0, 70, 50]), 'UPPER_RED1': np.array([10, 255, 255]), 'LOWER_RED2': np.array([170, 70, 50]), 'UPPER_RED2': np.array([180, 255, 255]), 'BLACK_PERCENT_THRESH': 50.0, 'GREEN_PERCENT_THRESH': 30.0, 'WHITE_PERCENT_THRESH': 50.0, 'RED_PERCENT_THRESH': 40.0}
        try: motor_control.setup_motors(); ultrassonico.setup_sensor()
        except Exception as e: print(f'Erro fatal ao iniciar hardware: {e}'); self.running = False
    def run(self):
        while self.running: self.handle_events(); self.update(); self.draw(); self.clock.tick(30) # Mantem o clock baixo
        self.quit_app()
    ...
    def draw(self):
        ...
        pygame.display.flip()
    def quit_app(self): print("Encerrando aplicação..."); self.tela_calibracao.stop(); self.tela_rodada.stop(); motor_control.full_stop_and_cleanup(); pygame.quit(); sys.exit()
class TelaInicio:
    def __init__(self, app): self.app = app; self.btn_iniciar = pygame.Rect(25, 250, 430, 80); self.btn_calibrar = pygame.Rect(25, 350, 430, 80); self.btn_sair = pygame.Rect(25, 450, 430, 80)
    def draw(self):
        self.app.screen.blit(self.app.logo, (SCREEN_WIDTH // 2 - 75, 50)); pygame.draw.rect(self.app.screen, GREEN_COLOR, self.btn_iniciar, border_radius=10); pygame.draw.rect(self.app.screen, PURPLE_COLOR, self.btn_calibrar, border_radius=10); pygame.draw.rect(self.app.screen, RED_PINK_COLOR, self.btn_sair, border_radius=10)
        texto_iniciar = self.app.font_grande.render("INICIAR RODADA", True, WHITE_COLOR); texto_calibrar = self.app.font_grande.render("CALIBRAR", True, WHITE_COLOR); texto_sair = self.app.font_grande.render("SAIR", True, WHITE_COLOR)
        self.app.screen.blit(texto_iniciar, texto_iniciar.get_rect(center=self.btn_iniciar.center)); self.app.screen.blit(texto_calibrar, texto_calibrar.get_rect(center=self.btn_calibrar.center)); self.app.screen.blit(texto_sair, texto_sair.get_rect(center=self.btn_sair.center))
...
class TelaCalibracao:
    def __init__(self, app): self.app = app; self.cap = None; self.frame = None; self.gray_frame = None; self.hsv_frame = None; self.step = 0; self.btn_proximo = pygame.Rect(SCREEN_WIDTH // 2 - 125, 550, 250, 60); self.camera_rect = pygame.Rect((SCREEN_WIDTH - CAMERA_DISPLAY_SIZE[0]) // 2, 120, CAMERA_DISPLAY_SIZE[0], CAMERA_DISPLAY_SIZE[1]); self.black_samples, self.green_samples, self.white_samples, self.red_samples = [], [], [], []
    def start(self):
        self.cap = cv2.VideoCapture(0, cv2.CAP_V4L2);
        if not self.cap.isOpened(): print("Erro: Webcam."); self.app.state = 'inicio'; return
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH); self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT); self.step = 0; self.black_samples, self.green_samples, self.white_samples, self.red_samples = [], [], [], []
    def stop(self):
    ...
    def draw(self):
        ...
        if self.frame is not None:
            ...
            pygame.draw.rect(self.app.screen, GRAY_COLOR, self.camera_rect); texto_cam = self.app.font_media.render("Sem Sinal da Câmera", True, WHITE_COLOR); self.app.screen.blit(texto_cam, texto_cam.get_rect(center=self.camera_rect.center))
        ...
        texto_instrucao = self.app.font_media.render(instrucao, True, TEXT_PURPLE_COLOR); self.app.screen.blit(texto_instrucao, (SCREEN_WIDTH // 2 - texto_instrucao.get_width() // 2, 420)); pygame.draw.rect(self.app.screen, PURPLE_COLOR, self.btn_proximo, border_radius=10); self.app.screen.blit(self.app.font_media.render(texto_botao, True, WHITE_COLOR), self.btn_proximo.center)
    def avancar_passo(self):
        ...
        elif self.step == 1 and self.green_samples: h_vals, _, _ = zip(*self.green_samples); self.app.calib_vars['LOWER_GREEN'] = np.array([max(0, min(h_vals) - 10), 40, 40]); self.app.calib_vars['UPPER_GREEN'] = np.array([min(179, max(h_vals) + 10), 255, 255])
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.btn_proximo.collidepoint(event.pos): self.avancar_passo()
                ...
                x_frame = int((event.pos[0] - self.camera_rect.x) * (FRAME_WIDTH / CAMERA_DISPLAY_SIZE[0])); y_frame = int((event.pos[1] - self.camera_rect.y) * (FRAME_HEIGHT / CAMERA_DISPLAY_SIZE[1]))
...
class TelaRodada:
    def __init__(self, app):
        ...
        self.ROI_CM = (131, 4, 58, 42); self.ROI_CE = (32, 65, 93, 42); self.ROI_CD = (195, 65, 93, 42)
        self.ROI_BE = (32, 137, 93, 42); self.ROI_BD = (195, 137, 93, 42)
        self.ZONAS = {'CM': self.ROI_CM, 'CE': self.ROI_CE, 'CD': self.ROI_CD, 'BE': self.ROI_BE, 'BD': self.ROI_BD}
        self.ROI_LINE_Y, self.ROI_LINE_HEIGHT = 120, 20
    ...
    def update(self):
        ...
        ret, captured_frame = self.cap.read() # Captura o frame original
        if not ret: self.acao = "Falha na Captura"; motor_control.stop_all_motors(); return
        
        # A lógica opera no 'captured_frame', a exibição usa o 'self.frame'
        ...
        if self.obstacle_state != "Nenhum":
            if self.obstacle_state == "Iniciando_Desvio": self.acao = "Obstaculo - Iniciar Desvio"; self.obstacle_state = "Contornando"
            elif self.obstacle_state == "Contornando": self.acao = "Obstaculo - Contornar"; self.obstacle_state = "Realinhando"
            elif self.obstacle_state == "Realinhando": self.acao = "Obstaculo - Realinhar"; self.obstacle_state = "Procurando_Linha"
            elif self.obstacle_state == "Procurando_Linha": self.acao = "Obstaculo - Procurar Linha"; self.obstacle_state = "Nenhum"
        ...
        # <<< OTIMIZAÇÃO: SÓ DESENHA NA TELA SE NÃO ESTIVER EM MODO COMPETIÇÃO >>>
        if not MODO_COMPETICAO:
    ...
    def draw(self):
        if self.frame is not None:
            # <<< OTIMIZAÇÃO: A CONVERSÃO DE CORES TAMBÉM PODE SER PULADA >>>
            if not MODO_COMPETICAO:
                frame_rgb = cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB)
            ...
            else: # Em modo competição, apenas mostra um retângulo cinza.
        ...
        # Os textos continuam sendo exibidos para dar feedback
