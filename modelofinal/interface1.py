...
class TelaCalibracao:
    def __init__(self, app):
        ...
        self.black_samples, self.green_samples, self.white_samples = [], [], []

    def start(self):
        self.cap = cv2.VideoCapture(0, cv2.CAP_V4L2)
        if not self.cap.isOpened():
            print("Erro: Não foi possível abrir a webcam.")
            self.app.state = 'inicio'; return
    ...
    def draw(self):
        ...
        else: instrucao, texto_botao = "Clique em vários pontos do Branco", "Finalizar Calibração"
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.btn_proximo.collidepoint(event.pos): self.avancar_passo()
                ...
                x, y = event.pos
                xf = int((x - self.camera_rect.x) * (FRAME_WIDTH / CAMERA_DISPLAY_SIZE[0]))
                yf = int((y - self.camera_rect.y) * (FRAME_HEIGHT / CAMERA_DISPLAY_SIZE[1]))
                if self.step == 0 and self.gray_frame is not None: self.black_samples.append(self.gray_frame[yf, xf])
                elif self.step == 1 and self.hsv_frame is not None: self.green_samples.append(self.hsv_frame[yf, xf])
                elif self.step == 2 and self.gray_frame is not None: self.white_samples.append(self.gray_frame[yf, xf])
...
class TelaRodada:
    def __init__(self, app):
        ...
        self.last_erro = 0
        self.gap_counter = 0
        self.MAX_GAP_FRAMES = 15 
        
        self.ROI_CM = (262, 8, 116, 85)
        self.ROI_CE = (64, 131, 186, 85)
        self.ROI_CD = (390, 131, 186, 85)
        self.ROI_BE = (64, 275, 186, 85)
        self.ROI_BD = (390, 275, 186, 85)
        self.ZONAS = {'CM': self.ROI_CM, 'CE': self.ROI_CE, 'CD': self.ROI_CD, 'BE': self.ROI_BE, 'BD': self.ROI_BD}
        
        self.ROI_LINE_Y = 240
        self.ROI_LINE_HEIGHT = 40
    ...
    def start(self):
        ...
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH)
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT)
        self.acao, self.erro = "Iniciando...", 0
        self.last_erro, self.gap_counter = 0, 0
    ...
    def update(self):
        ...
        cm, ce, cd, be, bd = zone_states.values()

        # <<< LÓGICA DE DECISÃO CORRIGIDA E PRIORIZADA >>>
        
        # --- PASSO 1: VERIFICAR CONDIÇÕES DE ALTA PRIORIDADE (CURVAS, VERDE, INTERSECÇÕES) ---
        is_maneuver = True # Flag para saber se uma ação especial foi tomada
        if be == "Preto" and bd == "Preto": self.acao = "Seguir em Frente (Intersecao T/Cruzamento)"
        elif bd == "Verde" and be == "Verde": self.acao = "Meia Volta"
            ...
            is_maneuver = False # Nenhuma manobra especial foi detectada

        # --- PASSO 2: SE NENHUMA MANOBRA ESPECIAL OCORREU, TRATAR SEGUIMENTO DE LINHA E GAPS ---
        if not is_maneuver:
            ...
            if M["m00"] > 0: # A linha foi encontrada
                self.acao = "Seguindo Linha"
                cx = int(M["m10"] / M["m00"])
                self.erro = cx - FRAME_WIDTH // 2
                self.last_erro = self.erro
                self.gap_counter = 0
            else: # A linha NÃO foi encontrada
                self.gap_counter += 1
                if self.gap_counter < self.MAX_GAP_FRAMES:
                    self.acao = "Atravessando Gap"
                    self.erro = self.last_erro # Usa o último erro conhecido
                else:
                    self.acao = "Procurando Linha"
                    self.erro = 0
        else:
            # Se estamos em uma manobra (curva/verde), o erro de seguimento de linha é irrelevante.
            self.erro = 0
            self.last_erro = 0 # Zera o last_erro para não influenciar após a curva

        # --- PASSO 3: ENVIAR A AÇÃO FINAL PARA O CONTROLE DO MOTOR ---
    ...
    def draw(self):
        ...
        texto_acao = self.app.font_media.render(self.acao, True, TEXT_PURPLE_COLOR)
        self.app.screen.blit(texto_acao, texto_acao.get_rect(centerx=SCREEN_WIDTH/2, y=450))
